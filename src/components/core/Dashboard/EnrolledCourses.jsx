    ...
    return (
        <>
            ...
            </div>
            {
                ...
                (
                    <div className="text-3xl font-medium place-content-center text-richblack-5 text-center">
                            ...
                            enrolledCourses.map((course,index,arr) =>{
                                ...
                                return (
                                    <div key={index} className={`flex items-center border border-richblack-700 ${index === arr.length - 1 ? "rounded-b-lg" : "rounded-none"}`}>
                                        <div className="flex w-[45%] md:flex-row flex-col cursor-pointer items-center gap-4 px-5 py-3"
                                        onClick={() => {
                                        navigate(
                                            `/view-course/${course?._id}/section/${course.courseContent?.[0]?._id}/sub-section/${course.courseContent?.[0]?.subSection?.[0]?._id}`
                                        )
                                        }}>
                                            <img src={`${course.thumbnail}`} alt="courseThumbnail" className="h-14 w-14 rounded-lg object-cover"/>
